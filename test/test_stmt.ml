...
let%test "test_stmt" =
  ...
  for _i = 0 to 100 do
    ...
    let sql = Printf.sprintf "SELECT * FROM test0" in
  ...
  for _i = 0 to 100 do
    (* Printf.printf "Create statement %d\n%!" i; *)
    let sql = Printf.sprintf "SELECT * FROM test0; SELECT * FROM test1;" in
  ...
  let sql = Printf.sprintf "SELECT * FROM test0; SELECT * FROM test0;" in
  let stmt = prepare db sql in
  print_endline "A-------------------------------------------";
  stepbystep stmt;
  print_endline "B-------------------------------------------";
  ...
  begin
    try
      assert_ok(bind_values stmt [ Data.INT 1L; Data.INT 2L ]);
    with
    |RangeError _ -> ()
    | exn -> raise exn;
  end;
  begin
    try
      assert_ok(bind_name stmt ":a" (Data.INT 3L))
    with
    | Not_found -> ()
    | exn -> raise exn;
  end;
  ...
  let (rc, sum) = fold stmt (fun s b -> Int64.add s (of_intdata b.(0))) 0L in
  assert_ok(finalize stmt);
  assert_ok(rc);
  assert(sum = 3L);
  Printf.printf "fold: sum of table0(b) is %Ld\n" sum;

  Gc.major (); (* Collect any dangling statements *)
  assert(db_close db);
  true
